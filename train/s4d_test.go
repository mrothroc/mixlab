...
func TestS4DSpecialWeightInitialization(t *testing.T) {
	shapes := []WeightShape{
		...
		{Name: "s4d_A_imag_scaled", Shape: []int{2, 3}, InitMode: "s4d_A_imag_lin", InitScale: 3},
	...
	for i, want := range wantImag {
		want *= 3
		if math.Abs(float64(weights[3][i]-want)) > 2e-6 {
			t.Fatalf("scaled A_imag[%d]=%g want %g", i, weights[3][i], want)
		}
	}
	...
	for i, value := range weights[5] {
		if value != 0 {
			t.Fatalf("B_imag[%d]=%g want 0", i, value)
		}
	}
	for i := 6; i < len(weights); i++ {
		nonzero := false
...
func TestS4DFrequencyScaleOnePreservesInitialization(t *testing.T) {
	baseline := initWeightData([]WeightShape{{
		Name: "s4d_A_imag", Shape: []int{2, 4}, InitMode: "s4d_A_imag_lin",
	}}, 19, "", 0)
	explicit := initWeightData([]WeightShape{{
		Name: "s4d_A_imag", Shape: []int{2, 4}, InitMode: "s4d_A_imag_lin", InitScale: 1,
	}}, 19, "", 0)
	for i := range baseline[0] {
		if baseline[0][i] != explicit[0][i] {
			t.Fatalf("A_imag[%d] default=%g explicit-one=%g", i, baseline[0][i], explicit[0][i])
		}
	}
}
