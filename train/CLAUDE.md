...
- `cuda_graph_limits.go` — pre-train hook: inspects the IR and sets `MLX_MAX_OPS_PER_BUFFER` / `MLX_MAX_MB_PER_BUFFER` env vars before MLX initializes
...
```
Controlled by `MIXLAB_MAMBA3_HOST_TIMING_START` (default 100) and `MIXLAB_MAMBA3_HOST_TIMING_EVERY` (default 100). Disable with `MIXLAB_DISABLE_MAMBA3_HOST_TIMING=1`. See `docs/canonical_mamba3.md` for the full env-var reference.
