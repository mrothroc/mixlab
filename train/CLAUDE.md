...
- **Data diagnostics** — `classification_data_diagnostics.go` samples early classification batches and warns once when the mean distinct-label count reaches less than 25% of the achievable diversity *above one* (`1 + 0.25*(achievable-1)`). The excess-over-one form matters: a plain 25%-of-achievable rule demands 2 of 2 labels in every binary batch, and fired on ~86% of correctly shuffled 5%-minority runs. It reports the mean/min/max from batches actually consumed, not how shards were written, so occasional 2-3-label shard-boundary batches do not hide near-degenerate ordering. See [`../data/CLAUDE.md`](../data/CLAUDE.md) for why balanced `class_counts` cannot detect this.
- **Liveness / debug capture** — `progress_file.go` publishes `{pid, step, optimizer_steps}` by atomic rename when `MIXLAB_PROGRESS_FILE` is set, for an out-of-process watchdog; it reports *committed* optimizer steps, so skipped updates do not read as progress, and it never samples the GPU. `debug_ptracer_linux.go` opts the trainer into `PR_SET_PTRACER` for one PID that must equal its actual parent — never `PR_SET_PTRACER_ANY`, never a host policy change; a denied prctl warns and continues rather than failing the run. Both are off unless their env vars are set. Consumer: [`../scripts/CLAUDE.md`](../scripts/CLAUDE.md).
