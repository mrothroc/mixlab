...
func TestGatedDeltaNetFullBlockMatchesReferenceFixture(t *testing.T) {
	if !mlxAvailable() {
		t.Skip("MLX backend not available")
	}
	fixtures := loadGatedDeltaNetFullBlockFixtures(t)
...
func loadGatedDeltaNetFullBlockFixtures(t *testing.T) []recurrentFullBlockFixture {
	t.Helper()
	path := filepath.Join("testdata", "gated_deltanet_full_block_reference.json")
