...
func TestGenerateDiffusionHybridBlockDiffusionMLXSmoke(t *testing.T) {
	if !mlxAvailable() {
		t.Skip("MLX backend not available")
	}

	cfg, err := ParseArchConfig([]byte(`{
		"name": "generate_hybrid_block_diffusion_mlx_smoke",
		"model_dim": 16,
		"vocab_size": 16,
		"seq_len": 4,
		"blocks": [
	...
	}`), "generate_hybrid_block_diffusion_mlx_smoke")
	if err != nil {
		t.Fatalf("ParseArchConfig: %v", err)
	}
	prog, err := buildGenerateDiffusionIRProgram(cfg)
	...
	result, err := generateDiffusionTokens(cfg, evaluator, []int{1}, 2)
	if err != nil {
		t.Fatalf("generateDiffusionTokens hybrid: %v", err)
	}
	if len(result.tokens) != 3 {
		t.Fatalf("generated token length = %d, want 3 (%v)", len(result.tokens), result.tokens)
	}
