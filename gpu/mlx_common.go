...
func (p *Program) AddOp(opType int, inputs, outputs []string, floatParams []float32, intParams []int) error {
	...
	if len(inputs) > 256 || len(outputs) > 3 || len(floatParams) > 4 || len(intParams) > 8 {
		return fmt.Errorf("IR op params exceed bridge limits; keep inputs<=256 outputs<=3 float_params<=4 int_params<=8")
	}
