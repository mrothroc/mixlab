...
func TestOptimizerStepGuardRollsBackNonFiniteGradientAndBiasStep(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if stats.LastGradientNonfinite == 0 && stats.LastStateNonfinite == 0 {
				t.Fatalf("stats did not attribute non-finite backward/update state: %+v", stats)
			}
			...
			if stats.AttemptedSteps != 2 || stats.CommittedSteps != 1 || stats.SkippedSteps != 1 || stats.LastStepSkipped {
