...
extern "C" __global__ void mamba3_selective_scan_checkpoints_v2(
    const float* x_flat,
    const float* dt_flat,
  ...
  for (int t = 0; t < T; ++t) {
    if ((t % window_size) == 0) {
      ...
      const int h_checkpoint_row = b * n_windows + w;
      const int phi_checkpoint_row = b * (n_windows + 1) + w;
      h_checkpoints[mamba3_state_idx(h_checkpoint_row, d, n0, D, N)] = h0;
      h_checkpoints[mamba3_state_idx(h_checkpoint_row, d, n1, D, N)] = h1;
      phi_checkpoints[mamba3_theta_idx(phi_checkpoint_row, d, k, D, K)] = phi;
    }
