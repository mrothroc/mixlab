...
std::vector<mlx_ir::OptimizerGroupConfig> read_optimizer_groups(
  ...
  for (int i = 0; i < n_optimizer_groups; ++i) {
    ...
    groups.push_back(mlx_ir::OptimizerGroupConfig{
        ...
        optimizer_groups[i].lamb_trust_ratio_cap,
        optimizer_groups[i].cautious_weight_decay != 0,
        optimizer_groups[i].cautious_weight_decay_activation_step,
...
int mlx_ir_trainer_set_program(int64_t trainer, int64_t program) {
  ...
  try {
    ...
    log_bridge_exception("mlx_ir_trainer_set_program", e);
    return -1;
  } catch (...) {
    std::cerr << "[mlx_bridge] mlx_ir_trainer_set_program unknown exception" << std::endl;
    return -1;
  }
}
...
int mlx_ir_trainer_optimizer_stats(
  ...
  if (!attempted_steps || !committed_steps || !skipped_steps ||
      !consecutive_skipped_steps || !last_step_skipped ||
  ...
  try {
    ...
    *consecutive_skipped_steps = t->consecutive_skipped_optimizer_steps;
