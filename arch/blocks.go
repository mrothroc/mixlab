...
func emitPlainAttentionIRWithKVOptionsEx(prog *Program, x string, wi, H, kvH, D, T, B, idx int, mlpMult float64, blockScales bool, dropout, attnDropout float32, skipAttention bool, qkGain float64, qkNorm bool, ropeDims int, xsa, sparseAttnGate bool, windowSize int, attentionMask, relativeAttention string, relativeWindow int, relativeParameterization string, kvSource int, kvCache map[int]BlockKVOutputs, blockIndex int) (int, error) {
	return emitPlainAttentionIRWithKVOptionsExConvention(prog, x, wi, H, kvH, D, T, B, idx, mlpMult, blockScales, dropout, attnDropout, skipAttention, qkGain, qkNorm, ropeDims, RopeConventionAdjacentPair, xsa, sparseAttnGate, windowSize, attentionMask, relativeAttention, relativeWindow, relativeParameterization, kvSource, kvCache, blockIndex)
}
...
func emitPlainAttentionIRWithKVOptionsExConventionNorm(prog *Program, x string, wi, H, kvH, D, T, B, idx int, mlpMult float64, blockScales bool, dropout, attnDropout float32, skipAttention bool, qkGain float64, qkNorm bool, differentialAttention bool, differentialLambdaInit *float64, ropeDims int, ropeConvention string, bias attentionBiasOptions, attnValueGate bool, attnPostNormMode string, xsa, sparseAttnGate bool, windowSize int, attentionMask, relativeAttention string, relativeWindow int, relativeParameterization string, kvSource int, kvCache map[int]BlockKVOutputs, blockIndex int, norm NormSpec, normPlacement string, ffnInternalNorm bool, ffnActivation string, ffnPreNorm, ffnBias bool, positionalEmbedding string, sharedRel sharedRelativeAttentionPlan, layerAgg *layerAggregationBuildState, segmentMask bool, adaLN *adaLNBuildState) (int, error) {
	...
	if differentialAttention {
		...
		if qkNorm || qkGain > 0 || attnValueGate || xsa || sparseAttnGate || kvSource > 0 {
			return wi, fmt.Errorf("differential_attention cannot be combined with qk_norm, qk_gain, attn_value_gate, xsa, sparse_attn_gate, or kv_source in v1")
		}
		...
		if attnPostNorm != PlainAttnPostNormNone {
			return wi, fmt.Errorf("differential_attention cannot be combined with attn_post_norm in v1")
		}
	}
	if qkGain < 0 {
	...
	if skipAttention {
		...
		if differentialAttention {
			wi += 5 // lambda vectors + per-head SubLN scale
		}
		if normalizeRelativeAttention(relativeAttention) == RelativeAttentionDebertaP2CC2P && normalizeRelativeAttentionParameterization(relativeParameterization) == RelativeAttentionParamPerBlockProjections {
		...
		// Q projection
		qWeightName, qBiasName, nextWI := emitLinearProjectionIR(prog, xAttn, wi, bias.QKV, q)
		wi = nextWI
		...
		if reuseKV {
			...
			// K/V projections
			kWeightName, kBiasName, nextWI := emitLinearProjectionIR(prog, xAttn, wi, bias.QKV, k)
			wi = nextWI
			if attnValueGate {
				_, _, nextWI = emitLinearProjectionIR(prog, xAttn, wi, bias.QKV, vRaw)
				wi = nextWI
				prog.Slice(vRaw, 0, kvProjDim, 1, 1, v)
				prog.Slice(vRaw, kvProjDim, kvProjDim+D, 1, 1, valueGate)
				prog.GELU(valueGate, valueGateAct)
			} else {
				_, _, nextWI = emitLinearProjectionIR(prog, xAttn, wi, bias.QKV, v)
				wi = nextWI
			}
			...
			if differentialAttention {
				lambdaInit := float32(effectiveDifferentialLambdaInit(differentialLambdaInit, blockIndex))
		...
		if reuseKV {
			if differentialAttention {
				return wi, fmt.Errorf("kv_source is not supported with differential_attention")
			}
