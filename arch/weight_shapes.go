...
func collectLinearFramesWeightShapesFromConfig(cfg *ArchConfig) ([]WeightMeta, error) {
	refs, err := normalizeWeightRefs(cfg.Blocks, cfg.Recurrence)
	if err != nil {
		return nil, fmt.Errorf("blocks: %w", err)
	}
	return collectLinearFramesWeightShapesWithRefs(cfg, refs)
}

func collectLinearFramesWeightShapesWithRefs(cfg *ArchConfig, refs []int) ([]WeightMeta, error) {
	metas, err := collectWeightShapesWithRefsHeadLayoutFeaturesNorm(
		cfg.ModelDim,
		cfg.InputAdapter.FeatureDim,
		cfg.SeqLen,
		cfg.EffectiveMLPMult(),
		false,
		cfg.BlockScales,
