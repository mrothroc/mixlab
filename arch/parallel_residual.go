...
func emitSequentialRangeWithRecurrenceDropout(prog *Program, specs []BlockSpec, refs []int, weightStarts []int, kvCache map[int]BlockKVOutputs, start, end int, stream, original string, wi, D, T, B, V int, opIdx *int, streamSeqLens map[string]int, mlpMult float64, blockScales, residMix, parallelResidual bool, dropout, attnDropout float32, backout *backoutBuildPlan, norm NormSpec, normPlacement string, ffnInternalNorm bool) (int, error) {
	plan, err := newParallelResidualPlan(specs, parallelResidual)
	if err != nil {
		return wi, err
	}
	...
	for i := start; i < end; {
		...
		wi, err = emitSequentialBlockWithRecurrenceDropout(prog, specs, refs, weightStarts, kvCache, i, stream, original, wi, D, T, B, V, opIdx, streamSeqLens, mlpMult, blockScales, residMix, dropout, attnDropout, norm, normPlacement, ffnInternalNorm)
		if err != nil {
			return wi, err
		}
		if backout != nil {
...
func emitSequentialOrderWithRecurrenceDropout(prog *Program, specs []BlockSpec, refs []int, weightStarts []int, kvCache map[int]BlockKVOutputs, order []int, stream, original string, wi, D, T, B, V int, opIdx *int, streamSeqLens map[string]int, mlpMult float64, blockScales, residMix, parallelResidual bool, dropout, attnDropout float32, backout *backoutBuildPlan, norm NormSpec, normPlacement string, ffnInternalNorm bool) (int, error) {
	plan, err := newParallelResidualPlan(specs, parallelResidual)
	if err != nil {
		return wi, err
	}
