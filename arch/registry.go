...
func init() {
	...
	RegisterBlock("moe", blockRegistration{
		Emitter: func(prog *Program, spec BlockSpec, stream string, wi, D, T, B, V, idx int, opts EmitOptions) (int, error) {
			return emitMoEIR(prog, spec, stream, wi, D, T, B, idx, opts.MLPMult, opts.BlockScales, opts.Dropout, opts.layerAgg, opts.adaLN, opts.BlockIndex)
		},
...
func plainWeightCount(spec BlockSpec, blockScales, residMix bool) (int, error) {
	...
	total += spec.attentionBiases().weightCount(spec.KVSource > 0)
	if spec.QKGain > 0 {
		total++
	}
	if spec.QKNorm {
