...
func plainWeightCount(spec BlockSpec, blockScales, residMix bool) (int, error) {
	...
	if spec.AttnBias {
		total += 2 // q and output projection biases
		if spec.KVSource <= 0 {
			total += 2 // k and value/value-gate projection biases
		}
	}
	if spec.QKGain > 0 {
		total++
	...
	case PlainAttnPostNormAfterOutProj, PlainAttnPostNormBeforeOutProj:
		total++ // explicit override with the legacy/default RMSNorm counter
	}
	if plainFFNActivationUsesGate(spec.FFNActivation) {
		total++
	}
	if spec.FFNPreNorm {
		total++ // legacy/default RMSNorm counter
	}
	if spec.FFNBias {
		total += 2
	}
