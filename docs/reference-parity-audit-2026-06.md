...
| Low | `s4d` | Regenerable fixtures from pinned `state-spaces/s4` v3.0.0 cover the minimal kernel plus the full grouped/trainable-B/bilinear/bidirectional primitive forward and every weight-gradient element; separate MLX/CPU finite-difference and FFT/scan checks remain | The fixture generator imports the upstream entry points and records source and environment provenance. It does not cover the surrounding Mixlab residual/normalization/GLU block composition against upstream, which is exercised by local oracles and HF parity instead. | Keep the generator and full-gradient fixture load-bearing when changing S4D parameterization, grouping, discretization, or bidirectional assembly. |
