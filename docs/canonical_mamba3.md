...
| `MIXLAB_MAMBA3_SCAN_BWD` | unset (`v1`) | Set to `v2` or `checkpoint-phi` to test the experimental backward CUDA path that checkpoints final window phase, accumulates A-log gradients locally, and reduces B/C gradients outside the recurrence kernel. |
