...
| `minimal_pair` | object | Required with an `energy` head | Disabled | Explicit minimal-pair data for native energy/ranking heads. V1 supports `source: "jsonl"` with `path`, `loss: "logistic"` or `"hinge"`, `margin`, and `pair_batch_fraction`. |
...
`training.minimal_pair` enables native energy/ranking heads inside multihead training. Pair JSONL records use `{"id":"...","clean":[...],"corrupt":[...],"family":"..."}` with token IDs in range. Energy heads pool each sequence row to one scalar energy, train clean/corrupt row pairs with logistic or hinge ranking loss, and report pair accuracy plus mean clean/corrupt energies. Lower energy is preferred. `score-ebm` reads native energy outputs for sequence or pair JSONL; HF export skips energy heads and exports the configured scorer head only.
