...
| `minimal_pair` | object | Required with an `energy` head | Disabled | Explicit minimal-pair data for native energy/ranking heads. Supports `source: "jsonl"` or compiled `source: "bin"` with `path`, `loss: "logistic"` or `"hinge"`, `margin`, and `pair_batch_fraction`. |
...
`training.minimal_pair` enables native energy/ranking heads inside multihead training. Pair JSONL records use `{"id":"...","clean":[...],"corrupt":[...],"family":"..."}` with token IDs in range. Use `mixlab -mode prepare-pairs` to validate JSONL and optionally compile it to a compact `.mpair` artifact, then set `source: "bin"` for faster training startup. `mixlab -mode prepare` can also generate corpus-only pair JSONL with weighted broad corruption families (`agreement`, `attractor`, `word_order`, `npi_licensor`, `quantifier_scope`, and `filler_gap`), induced morphology tables, duplicate filtering, rejection counters, and optional audit samples. Energy heads pool each sequence row to one scalar energy, train clean/corrupt row pairs with logistic or hinge ranking loss, and report pair accuracy plus mean clean/corrupt energies. Lower energy is preferred. `score-ebm` reads native energy outputs for sequence or pair JSONL and emits aggregate plus per-family accuracy summaries for pair input; HF export skips energy heads and exports the configured scorer head only.
