...
| `batch_tokens` | integer | No | `1024` | Tokens per optimization step. Must be divisible by `seq_len` normally; with `length_buckets`, it is a ceiling used to derive each bucket's row count. |
| `length_buckets` | integer array | No | omitted | Optional strictly increasing classification sequence widths. Supported for `linear_frames` and `discrete_codebooks`; each width must be `<= seq_len` and the largest must be `<= batch_tokens`. |
