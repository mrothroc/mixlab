...
| `heads` | array | Required for `objective: "multihead"` | None | Per-head objective specs for shared-trunk multihead training. V1 supports head objectives `"causal"`, `"mlm"`, `"mntp"`, and `"block_diffusion"`. |
| `export_head` | string | No | First non-diffusion head | Multihead scorer head exported by `export-hf`. Must not name a `block_diffusion` head. |
| `diffusion_head` | string | No | First block-diffusion head | Multihead denoiser head used by `generate-diffusion` and `score-diffusion`. |
