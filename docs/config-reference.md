...
| `heads` | array | Required for `objective: "multihead"` | None | Per-head objective specs for shared-trunk multihead training. V1 supports head objectives `"causal"`, `"mlm"`, `"mntp"`, `"block_diffusion"`, and `"rtd"`. |
| `export_head` | string | No | First non-native-only scorer head | Multihead scorer head exported by `export-hf`. Must not name a `block_diffusion` or `rtd` head. |
...
| `rtd` | object | No | Disabled | ELECTRA-style replaced-token detection auxiliary for multihead training. V1 supports tied-generator RTD using an MLM/MNTP generator head and a binary detector head. |
...
`training.objective: "multihead"` requires at least two uniquely named heads and a positive total raw `loss_weight`. Each head supports `objective`, `loss_weight`, `layer_aggregation` (`"none"` or native-only `"dwa"`), `output_head` (`"bert_mlm"` for MLM/MNTP, `"linear"`, or `"binary"` for RTD), `tie_embeddings`, `final_norm`, and an optional per-head `diffusion` object for `block_diffusion` heads. Multihead v1 rejects top-level `training.diffusion`, distillation, data2vec, MTP, first-byte masked loss, example framing, segment attention masking, recurrence phases, custom blocks, and `plain.window_size`. `export-hf` exports only `export_head`; head-level DWA is currently native-only because it is not equivalent to the top-level HF DWA template. `generate-diffusion` and `score-diffusion` use `diffusion_head`.

`training.rtd` enables ELECTRA-style replaced-token detection inside multihead training. V1 supports `generator: "tied"` only: `generator_head` names an MLM or MNTP head whose detached logits sample replacements, and exactly one head must set `objective: "rtd"` with `output_head: "binary"`. `mask_prob` defaults to `mlm_mask_prob`, `sample_temperature` defaults to `1.0`, and `discriminator_loss_weight` defaults to `50.0`; the RTD loss contribution is `head.loss_weight * discriminator_loss_weight * BCE`. The detector predicts `1=original`, `0=replaced` over every token. HF export skips RTD state and exports the configured scorer head only; `score-electra` reads native detector logits and reports summed `log P(original)` scores for token-id sequences.
