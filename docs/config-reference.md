...
- `attn_bias` — legacy shorthand enabling/disabling learned zero-initialized biases on all Q/K/V/O projections (default `false`). Cannot be specified together with either split field, even when explicitly `false`. Existing configs and checkpoint weight order are unchanged.
- `attn_qkv_bias` — independent `plain` Q/K/V projection biases (default `false`); a `kv_source` consumer allocates only its own Q bias. GQA uses the actual K/V projection widths, including the widened V projection when `attn_value_gate` is enabled. With `relative_attention_parameterization: "shared_qk_reuse"`, Q/K biases are also reused for the shared relative table projections.
- `attn_out_bias` — independent `plain` output-projection bias (default `false`). For example, `{"type":"plain","heads":8,"attn_qkv_bias":false,"attn_out_bias":true}` adds only an output bias, with `model_dim` extra parameters per block. Split biases are zero-initialized; nonzero reference initialization is not implied. Both split controls enabled are equivalent to `attn_bias: true`.
...
| `min_lr_fraction` | number | No | legacy when omitted | Must be in `[0,1)`. Omitted preserves the historical cosine endpoint of `0.1 * lr`, or `0.01 * lr` with warmdown. Explicit `0` decays to zero; a positive fraction sets the cosine endpoint and floors warmdown (not warmup). Phase schedules retain their 1% warmdown target unless a higher floor or explicit zero is configured. |
