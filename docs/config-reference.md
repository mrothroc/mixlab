...
| `minimal_pair` | object | Required with an `energy` head | Disabled | Explicit minimal-pair data for native energy/ranking heads. Supports `source: "jsonl"` or compiled `source: "bin"` with `path`, `loss: "logistic"` or `"hinge"`, `margin`, `pair_batch_fraction`, and `energy_aggregation: "mean"` or `"differing_span"`. |
