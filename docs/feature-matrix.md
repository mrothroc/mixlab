...
| Discrete codebook preparation | Yes | `mixlab -mode prepare -input-format codebooks` | Ingests integer `.npy`/`.npz` `[N,T,Q]` codec arrays, validates IDs against an explicit codebook vocabulary, and writes atomic labeled shards with optional valid lengths. |
...
| Continuous sequence classification | Yes | Same native classification metrics | No | CLS head/middle/tail supports bidirectional attention, canonical Mamba-3, Gated DeltaNet, and S4D natively. HF supports existing S4D/attention exports; recurrent gates and padding restrictions still apply. A matching continuous manifest is required for training. | [S4D reference](../examples/continuous_s4d_lra_image_reference.json), [CLS attention](../examples/cls_frames_classifier.json) |
| Image patch classification | Yes, crop/flip in the loader | Unaugmented full-image records | No | `linear_patches` adds geometry and optional learned XY; tokenizer-free HF export follows supported backbone restrictions. | [Patch classifier](../examples/linear_patches_classifier.json), [guide](image-patches.md) |
| Discrete multi-codebook classification | Yes | Same native classification metrics | No | Native-only in v1. Uses shared offset embeddings plus codebook-attention or mean fusion; a matching codebook manifest is required. | [discrete_codebooks_gated_deltanet_classification_tiny.json](../examples/discrete_codebooks_gated_deltanet_classification_tiny.json) |
...
| BatchNorm running statistics | Native fixed-shape classification on core/FFN, S4D, canonical Mamba-3, and Gated DeltaNet outer norms | Affine pre-norm only; recurrent mixer-internal norms remain canonical; padded records, recurrence, SWA, eval TTT, and HF export are gated | [S4D reference](../examples/continuous_s4d_batchnorm_reference_tiny.json), [Mamba comparison arm](../examples/continuous_mamba3_batchnorm_comparison_tiny.json) |
