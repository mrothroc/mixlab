...
| Continuous sequence classification | Yes | Same native classification metrics | No | `linear_frames` exports support fixed-shape S4D-only classifiers and padding-aware CLS attention classifiers. A matching continuous manifest is required for training. | [S4D reference](../examples/continuous_s4d_lra_image_reference.json), [CLS attention](../examples/cls_frames_classifier.json) |
