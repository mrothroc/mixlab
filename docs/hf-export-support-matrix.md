...
| `training.objective="hybrid"` | Supported for MLM/MNTP secondary objectives | Exports both the causal evaluation graph and a masked-LM graph. Block-level `attention_mask` values are overridden by head: causal for `AutoModelForCausalLM`, bidirectional for `AutoModel` and `AutoModelForMaskedLM`. Hybrid block-diffusion checkpoints are rejected until diffusion export semantics are defined. |
...
| `training.objective="block_diffusion"` | Unsupported | Native block-diffusion generation uses objective-specific block masks and sampler policy that are not represented in the current Hugging Face template. |
