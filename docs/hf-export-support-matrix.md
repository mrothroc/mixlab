...
| `training.objective="hybrid"` | Supported | Exports both the causal evaluation graph and a masked-LM graph. Block-level `attention_mask` values are overridden by head: causal for `AutoModelForCausalLM`, bidirectional for `AutoModel` and `AutoModelForMaskedLM`. |
