...
|---------|--------|-------|
| `plain` attention | Supported | Adjacent-pair RoPE by default, optional `rope_convention: "half_rotation"`, partial `rope_dims`, GQA through `kv_heads`, `qk_norm`, `qk_gain`, XSA projection, sparse attention gates, `causal`/`bidirectional`/`none` masks, and causal `window_size`. Top-level `positional_embedding: "learned_absolute"` exports WPE and disables RoPE; `"none"` disables RoPE without WPE. |
...
| `plain.ffn_activation=gelu` / `gelu_new` | Supported | Non-gated FFN tails use exact GELU or GPT-2 tanh-approx GELU respectively. `plain.ffn_pre_norm` exports a second pre-FFN norm, and `plain.ffn_bias` exports FFN projection biases. |
...
| `hf_export_format="gpt2"` | Supported for strict GPT-2 stacks | Writes a native `model_type: "gpt2"` / `GPT2LMHeadModel` directory with packed QKV tensors, WTE/WPE, GPT-2 block names, affine LayerNorms, and tied `lm_head.weight`. Rejected unless the config is causal, sequential `plain`, learned absolute positions, affine LayerNorm, tied embeddings, attention/FFN biases, second pre-FFN norm, and no Mixlab extras. |
| `AutoModel` backbone | Supported | `config.json` includes an `AutoModel` mapping to `MixlabModel`, which returns `last_hidden_state` before the LM head. Causal-only exports use causal blocks; masked-capable MLM/MNTP/hybrid exports use `masked_blocks` as the bidirectional encoder backbone. The generated template consumes Hugging Face `attention_mask` for padded batches. |
| `AutoModelForMaskedLM` | Supported for masked-capable checkpoints | MLM, MNTP, and hybrid exports register `MixlabForMaskedLM`. The default linear masked head reuses the exported `lm_head_weight`; `mlm_head="bert"` uses its transform stack and tied embedding output. Masked exports compute unshifted masked-label cross entropy with `-100` ignored labels and construct plain attention blocks with bidirectional masks. The exporter also writes the tokenizer `mask_token`/`mask_token_id` (resolved from `training.mlm_mask_token_id` against the tokenizer vocab) so masked/MNTP eval works without manual tokenizer fixups. |
| `mlm_head="bert"` | Supported for masked-capable checkpoints | `MixlabForMaskedLM` applies the BERT-style no-affine LayerNorm, dense+bias, GELU, no-affine LayerNorm, dropout, and tied embedding output plus output bias. The exported causal class still uses `lm_head_weight` so one directory can load through both causal and masked auto classes. |
...
| `training.objective="hybrid"` | Supported | Exports the causal evaluation graph. Hybrid MLM/MNTP checkpoints also export a masked-LM graph; block-level `attention_mask` values are overridden by head: causal for `AutoModelForCausalLM`, bidirectional for masked-capable `AutoModel` and `AutoModelForMaskedLM`. Hybrid block-diffusion checkpoints export the causal view only and drop the native-only diffusion sampler/mask. |
| `training.objective="mlm"` or `"mntp"` | Supported | Exports both heads. `AutoModel`/`AutoModelForMaskedLM` are the intended bidirectional encoder and masked-eval paths; `AutoModelForCausalLM` remains available for consumers that need next-token logits from the same checkpoint. |
| `training.objective="block_diffusion"` | Unsupported | Pure block-diffusion checkpoints use objective-specific block masks and sampler policy that are not represented in the current Hugging Face template. Use a hybrid causal + block-diffusion checkpoint when a causal HF export is needed. |
| `training.objective="multihead"` | Supported for scorer-only export | Exports the configured non-native-only `export_head` plus the shared trunk and skips native-only block-diffusion denoiser and RTD detector/generator weights. `AutoModelForMaskedLM` is registered when the scorer head is MLM/MNTP with `output_head: "bert_mlm"`. Head-level DWA is native-only in v1 and fails clearly on HF export. |
