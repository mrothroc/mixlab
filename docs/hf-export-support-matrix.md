...
| `AutoModelForMaskedLM` | Supported for masked-capable checkpoints | MLM, MNTP, and hybrid exports register `MixlabForMaskedLM`. It reuses the same exported `lm_head_weight`, computes unshifted masked-label cross entropy with `-100` ignored labels, and constructs plain attention blocks with bidirectional masks. The exporter also writes the tokenizer `mask_token`/`mask_token_id` (resolved from `training.mlm_mask_token_id` against the tokenizer vocab) so masked/MNTP eval works without manual tokenizer fixups. |
