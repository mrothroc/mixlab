...
| `training.objective="multihead"` | Supported for scorer-only export | Exports the configured non-native-only `export_head` plus the shared trunk and skips native-only block-diffusion denoiser or RTD detector weights. `AutoModelForMaskedLM` is registered when the scorer head is MLM/MNTP with `output_head: "bert_mlm"`. Head-level DWA is native-only in v1 and fails clearly on HF export. |
