...
- **Minimal-pair ranking**: Use `multihead_mntp_energy_tiny.json` with a `training.minimal_pair` JSONL file when you want a native energy head to rank clean sequences below corrupted variants. Set `energy_aggregation: "differing_span"` when your pair data has local edits and you want energy pooled over explicit or derived edit spans. `export-hf` exports the scorer head only; `score-ebm` reads native sequence energies.
