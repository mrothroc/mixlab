...
- **Nonlinear test-time-training layer**: Use `ttt_mlp_tiny.json` to exercise the native causal TTT-MLP token mixer. Training resets its inner MLP per sequence; eligible native generation stacks retain request-owned MLP, partial-gradient, and Q/K convolution state across prefill/decode. HF export remains gated.
