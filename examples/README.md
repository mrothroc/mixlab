...
| [continuous_s4d_length_bucketed_classification_tiny.json](continuous_s4d_length_bucketed_classification_tiny.json) | Length-bucketed S4D classifier | Skewed-length records trained at four bucket widths instead of one padded width |
| [discrete_codebooks_linear_probe_tiny.json](discrete_codebooks_linear_probe_tiny.json) | Discrete codec-token linear probe | Learned codebook adapter, temporal mean pooling, and classifier with no sequence blocks |
| [discrete_codebooks_linear_probe_newbob_tiny.json](discrete_codebooks_linear_probe_newbob_tiny.json) | NewBob codec-token linear probe | Full-split validation-driven LR annealing on a block-free classification probe |
| [discrete_codebooks_gated_deltanet_classification_tiny.json](discrete_codebooks_gated_deltanet_classification_tiny.json) | Discrete codec-token classifier | Shared offset codebook embeddings, learned codebook fusion, and a Gated DeltaNet backbone |
| [s4d_sobolev_controls/](s4d_sobolev_controls/) | Matched Sobolev beta controls | Unbounded, bounded, fixed, and lower-LR beta smoke recipes |
...
- **Native sequence classification**: Use `sequence_classification_gated_deltanet_tiny.json` with labeled JSONL prepared using `-label-field` or FASTA prepared using `-label-file`. Use `sequence_classification_bidirectional_mamba3_tiny.json` when every recurrent layer should use both sides of the valid sequence; this path is native-only in v1. Warm-start from the matching LM architecture with `-safetensors-load`; Mixlab appends a fresh classifier and fine-tunes the full native backbone.
- **Continuous sequence classification**: Use `continuous_mamba3_classification_tiny.json` for a selective recurrent baseline or `continuous_s4d_classification_tiny.json` for an LTI FFT-convolution baseline with `[N,T,F]` arrays prepared using `-input-format continuous`. Match `input_adapter.feature_dim`, `seq_len`, and `training.classification.num_labels` to the generated manifest. When record lengths are skewed, `continuous_s4d_length_bucketed_classification_tiny.json` adds `training.length_buckets` so each batch runs at the smallest width that fits it; prepare those shards with `-length-file` (or an `.npz` `lengths` array) so the valid lengths reach the loader. See [Length-bucketed classification](../docs/config-training.md#length-bucketed-classification).
- **Discrete codec-token classification**: Use `discrete_codebooks_linear_probe_tiny.json` for an adapter-plus-pooling probe, or `discrete_codebooks_gated_deltanet_classification_tiny.json` for a sequence-mixer model. Both consume integer `[N,T,Q]` arrays prepared using `-input-format codebooks` and explicitly select `weight_init: "pytorch_linear"` for PyTorch classifier parity. Match `num_codebooks`, `codebook_vocab_size`, `seq_len`, and label count to the generated manifest. This path is native-only in v1.
- **Metric-driven classification LR**: Use `discrete_codebooks_linear_probe_newbob_tiny.json` when a reference recipe anneals from full-split validation loss or error rate. Set `val_every_steps` to the reference validation boundary and keep `val_examples: 0` for the full finite split.
