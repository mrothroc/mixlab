...
- **RTD auxiliary training**: Use `multihead_mntp_rtd_tiny.json` when you want one shared trunk trained with an MNTP scorer head plus an ELECTRA-style replaced-token detector. `export-hf` exports the scorer head only; `score-electra` reads native detector scores.
