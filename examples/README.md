...
- **Discrete codec-token classification**: Use `discrete_codebooks_gated_deltanet_classification_tiny.json` with integer `[N,T,Q]` arrays prepared using `-input-format codebooks`. Match `num_codebooks`, `codebook_vocab_size`, `seq_len`, and label count to the generated manifest. This path is native-only in v1.
